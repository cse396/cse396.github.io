...
from Control import autonom_rob
...
class Server:
    ...
    def send_mapping_data(self):
        ...
        i_sign=1
        while True:
            command=str(lidar_servo.get_sonic_mapping(i, 2*(back_i)+200, 2*(back_i)+20))+"\n"
            self.send_data(self.connection2, command)
            i += 5
            back_i_inc=5*math.cos(math.radians(i))*i_sign
            back_i+=back_i_inc
            if back_i>=70:
                i_sign=-1
            elif back_i<=-110:
        ...
        while True:
            ...
            for oneCmd in cmdArray:
                ...
                if data==None or data[0]=='':
                    ...
                    self.send_data(self.connection1,command)
                elif cmd.CMD_CTRL in data:
                    #ctrl = os.system('sudo python Control.py')
                    if self.auton:
                        self.auton_event.set()
                        self.auto_proc.join()
                        print('autonom off')
                        self.auton_event=Event()
                        self.auton=False
                    else:
                        self.auto_proc=Process(target=autonom_rob, args=(self.auton_event,))
                        self.auto_proc.start()
                        self.auton=True
                        print('autonom on')
                else:
